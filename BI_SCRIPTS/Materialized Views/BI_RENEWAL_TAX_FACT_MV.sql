CREATE MATERIALIZED VIEW BIADMIN.BI_RENEWAL_TAX_FACT_MV 
TABLESPACE USERS
PCTUSED    0
PCTFREE    10
INITRANS   2
...
WITH PRIMARY KEY
AS 
/* Formatted on 2016/08/31 11:21 (Formatter Plus v4.8.8) */
SELECT                                                   --ROWNUM policy_code,
       a.policy_no policy_no, c.line_code line_code,
       ...
       h.branch_code, a.branch_name_dist, a.eff_date, a.check_issue_date,
       ...
       a.reinstate_tag, new_policy_id,
--       CASE
--          WHEN d.share_premium = 0 THEN 0
--          ELSE (NVL (d.share_percentage, 100) / 100) * a.prem_renew_amt
--       END
                                      a.prem_renew_amt, renewal_tag,
       a.expiry_tag, a.prem_amt,
       TO_CHAR (a.max_exp_date, 'YYYYMMDD') max_expiry_date
  FROM bi_renewal_tax_fact_mv_tmp a,
       cg_ref_codes b,
       bi_line_dim_mv c,
 ...
 WHERE b.rv_domain = 'GIPI_POLBASIC.POL_FLAG'
   ...
   AND a.branch_name_dist = h.branch_name(+);

COMMENT ON MATERIALIZED VIEW BIADMIN.BI_RENEWAL_TAX_FACT_MV IS 'snapshot table for snapshot BIADMIN.BI_RENEWAL_TAX_FACT_MV';
